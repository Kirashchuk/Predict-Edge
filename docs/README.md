...
- `data/` - локальне JSON-сховище для user-created market metadata.
...
| [ADR-002-bun-vite-stack.md](ADR-002-bun-vite-stack.md) | Рішення про Bun/Hono backend, Vite frontend і Hardhat contract tooling |
...
| AMM | `x*y=k`, fee `200 bps`, seed liquidity `1 USDC` у базовому deploy |
...
| Backend | Hono API на `http://localhost:8787`, routes `/v1/markets`, `/docs`, `/openapi.json` |
...
| Дані | `data/markets.json` для створених ринків |
...
Після `bun run deploy` адреси пишуться в root `.env.local` як `DEPLOY_*`. Команда
...
- Limit orders у поточному UI йдуть через `OnChainLimitOrderBook`.
